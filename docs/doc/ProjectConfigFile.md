...
The configuration file is a TOML file which accepts the parameters listed below.
The default values are adopted if the config file cannot be found or if their values are not provided in the config file. You can tell Lince where to find the config file using the function `LinceAppSetConfigFile(string_t path)` which must be called before calling `LinceRun()`. If the path is not provided, it is set by default to `project.toml` which will look in the current working directory.


| Parameter            | Type | Description                                           | Default       |
| -------------------- | ---- | ----------------------------------------------------- | ------------- |
| `root`               | str  | Absolute path to the root directory of the project.   | `"./"`        |
| `assets`             | str  | Path to the app's assets, relative to root.           | `"assets/"`   |
| `engine`             | str  | Path to the engine's assets, relative to root.        | `"lince/"`    |
