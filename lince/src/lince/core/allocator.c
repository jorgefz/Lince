...
static void* LinceSTBIImageAlloc(size_t size)                { return LinceMemoryAllocTagged(size,    LinceAllocTag_Assets, 0, "stb_image.c", "<stb_image function>"); }
static void* LinceSTBIImageRealloc(void* block, size_t size) { return LinceMemoryRealloc    (block, size,                   0, "stb_image.c", "<stb_image function>"); }
static void  LinceSTBIImageFree(void* block)                 {        LinceMemoryFree       (block,                         0, "stb_image.c", "<stb_image function>"); }

/* Memory management interface for toml */
static void* LinceTOMLAlloc(size_t size) { return LinceMemoryAllocTagged(size, LinceAllocTag_Assets, 0, "toml.c", "<toml function>"); }
static void  LinceTOMLFree(void* block)  {        LinceMemoryFree       (block,                      0, "toml.c", "<toml function>"); }
...
void LinceAllocatorInit(void){
    ...
    LINCE_INFO("[ALLOC] Allocator initialised");
}

/** @brief Uninitialise Allocator */
uint64_t LinceAllocatorUninit(void){
    LINCE_INFO("[ALLOC] Peak of %lu blocks allocated (%.2f kb)",
    ...
    if(unfreed_blocks != 0){
        LINCE_WARN("[ALLOC] %u unfreed blocks (%u bytes)",
    ...
    LINCE_INFO("[ALLOC] Allocator uninitialised");
...
void LinceAllocatorSet(LinceAllocFn alloc_fn, LinceReallocFn realloc_fn, LinceFreeFn free_fn, void* user_data){
    if(_global_allocator.initialised){
        LINCE_WARN("[ALLOC] Cannot set allocator after it has been initialised");
        return;
    }
...
#if defined(LINCE_DEBUG)
    LinceAllocHeader* header = _global_allocator.alloc(size + sizeof(LinceAllocHeader), _global_allocator.user_data);
    if(header == NULL){
        LINCE_ERROR("[ALLOC] Failed to allocate block of %ld bytes at %s:%d in function '%s'",
            size + sizeof(LinceAllocHeader), file, line, func);
        exit(-1);
    }
...
void* LinceMemoryRealloc(void* block, size_t size, int line, const char* file, const char* func){
    ...
    if(header->allocator != &_global_allocator){
        LINCE_ERROR("[ALLOC] Attempting to reallocate invalid or corrupted heap pointer 0x%p for %s at %s:%d in function '%s'",
            block, LinceAllocatorGetTagStringName(header->tag), file, line, func);
        exit(-1);
    }
    ...
    if(new_header == NULL){
        LINCE_ERROR("[ALLOC] Failed to allocate block of %ld bytes for %s at %s:%d in function '%s'",
            size + sizeof(LinceAllocHeader), LinceAllocatorGetTagStringName(header->tag), file, line, func);
        exit(-1);
    }
...
#if defined(LINCE_DEBUG)
    if(!block){
        LINCE_WARN("[ALLOC] Free called on NULL pointer at %s:%d in function '%s'", file, line, func);
        return;
    }

    LinceAllocHeader* header = (LinceAllocHeader*)block - 1;
    if(header->allocator != &_global_allocator){
        LINCE_WARN("[ALLOC] Attempted to free invalid or corrupted heap pointer 0x%p at %s:%d in function '%s'",
            block, file, line, func);
        return;
    }
